...
const toDate = (val: string | Date): Date => (val instanceof Date ? val : new Date(val));

const formatMonthYear = (val: string | Date): string =>
  toDate(val).toLocaleDateString(undefined, { year: "numeric", month: "short" });

const formatDuration = (from: string | Date, to: "present" | string | Date) => {
...
export default function Experience() {
  ...
  useEffect(() => {
    const fetchAbout = async () => {
      ...
      const list: ExperienceItem[] = Array.isArray(about?.experience) ? about.experience : [];
      setExperience(list);
    };
    fetchAbout();
  }, []);
  ...
  return (
    <section className="rounded-xl border border-white/5 bg-white/2 p-6 backdrop-blur-sm">
      <h2 className="text-3xl text-center font-semibold text-white mb-6">Experience</h2>
      ...
      <div className="relative">
        ...
        <ul className="space-y-6">
          {items.map((exp, idx) => {
            ...
            const endLabel = isPresent ? "Present" : formatMonthYear(exp.endDate as string | Date);
            const duration = formatDuration(exp.startDate, exp.endDate);
            const company = exp.companyName;
            const locationLabel = exp.location ? exp.location.charAt(0).toUpperCase() + exp.location.slice(1) : "";
            return (
                ...
                <div className="rounded-lg border border-white/10 bg-white/5 p-4">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div className="text-white font-medium">
                      {exp.role} <span className="text-gray-400">@ {company}</span>
                    </div>
                    <div className="text-gray-400 text-sm">
                      ...
                      {duration && (
                        <>
                          ...
                          <span className="text-emerald-400/90">{duration}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  {locationLabel && (
                    <div className="text-gray-500 text-sm mt-1">{locationLabel}</div>
                  )}

                  {Array.isArray(exp.jobTechStack) && exp.jobTechStack.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3 items-center">
                      <h4 className="text-white font-light">Tech i am working with</h4>
