...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[50] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex",
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-white transition duration-200 hover:text-white lg:flex lg:space-x-2 pointer-events-none",
        className,
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 text-white h-full w-full rounded-full bg-neutral-800"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <X className="z-[50] text-white" onClick={onClick} />
  ) : (
  ...
  return (
    ...
    >
      <img
        ...
        height={30}
      />
      <span className="font-medium text-white">Niloy</span>
    </a>
  );
};
